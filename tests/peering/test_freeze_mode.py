...
async def test_toggled_on_for_higher_priority_peer_when_initially_off(
        k8s_mocked, caplog, assert_logs, settings, looptime, event_loop,
        peering_resource, peering_namespace):
    ...
    event_loop.call_later(1.23, stream_pressure.set)
...
async def test_resumes_immediately_on_expiration_of_blocking_peers(
        k8s_mocked, caplog, assert_logs, settings, priority, looptime, event_loop,
        peering_resource, peering_namespace):
