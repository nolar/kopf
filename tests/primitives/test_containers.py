...
async def test_does_not_wake_up_when_reset(event_loop, looptime):
    container = Container()

    async def reset_it():
        await container.reset()

    event_loop.call_later(1, asyncio.create_task, reset_it())
    with pytest.raises(asyncio.TimeoutError):
        await asyncio.wait_for(container.wait(), timeout=9)
    assert looptime == 9
...
async def test_wakes_up_when_set(event_loop, looptime):
    container = Container()
...
async def test_iterates_when_set(event_loop, looptime):
    container = Container()

    async def set_it(v):
        await container.set(v)

    event_loop.call_later(6, asyncio.create_task, set_it(123))
    event_loop.call_later(9, asyncio.create_task, set_it(234))
